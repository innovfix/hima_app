...
//                        addCoins(userId, coinId)  // Retry once if it failed
    ...
    fun addCoinsGpay(userId: Int, coinId: String, status: Int, orderId: String, massage: String,) {
        _navigateToMain.postValue(false)
        viewModelScope.launch {
            walletRepositories.addCoinsGpay(userId, coinId, status, orderId, massage,  object : NetworkCallback<AddCoinsResponse> {
                override fun onResponse(call: Call<AddCoinsResponse>, response: Response<AddCoinsResponse>) {
                    Log.d("addCoins", "Raw Response: ${response.body()?.toString()}")
...
//                        addCoins(userId, coinId)  // Retry once if it failed
    ...
    fun add_coins_cashfree(userId: Int, coinId: String, status: Int, orderId: String, massage: String,) {
        _navigateToMain.postValue(false)
        viewModelScope.launch {
            walletRepositories.add_coins_cashfree(userId, coinId, status, orderId, massage,  object : NetworkCallback<AddCoinsResponse> {
                override fun onResponse(call: Call<AddCoinsResponse>, response: Response<AddCoinsResponse>) {
                    Log.d("addCoins", "Raw Response: ${response.body()?.toString()}")
