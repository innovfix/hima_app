...
object FcmUtils {
    ...
    fun clearUserBusyStatus() {
        ...
        // "userBusy" delivery, so the observer would fire with stale data and
        // bounce the caller to AgoraRandomCallActivity without sending
        // callDeclined — leaving the female ringing.
        if (Looper.myLooper() == Looper.getMainLooper()) {
